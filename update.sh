...
if [ "$RES" != "" ] || [ "$RES1" == "" ] ; then
	...
	sudo docker run --restart=always --privileged=true --name flexget --link transmission -p 5050:5050 -e TRAKT_USERNAME=$TRAKT_USERNAME -e TRAKT_ACCOUNT=$TRAKT_ACCOUNT -v $HOME/.flexget:/root/.flexget -v $MEDIA_PATH:/root/Storage tokko/flexget:latest "/root/flexget.sh" "$1" &
else
...
if [ "$RES" != "" ] || [ "$RES1" == "" ] ; then
	sudo docker run --link transmission -d --restart=always -p 8989:8989 --name sonar -v $MEDIA_PATH:/root/Storage -v $HOME/.sonarr:/root/.config/NzbDrone tokko/sonarr:latest
else
